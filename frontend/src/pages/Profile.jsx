...
function StatCard({ label, value, accent = false, testid }) {
  return (
    <div className="glass rounded-lg p-5" data-testid={testid}>
      ...
      <div className={`mt-2 font-hud text-3xl ${accent ? "text-[#2B4FFF] glow-text" : "text-white"}`}>{value}</div>
    </div>
  );
}
    ...
    <div className="min-h-[calc(100vh-3.5rem)] scanline">
      <div className="max-w-6xl mx-auto px-6 py-10">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-5">
          {user.picture ? (
            <img src={user.picture} alt="" className="w-20 h-20 rounded-full border-2 border-[#2B4FFF]/50 glow-box" />
          ) : (
            <div className="w-20 h-20 rounded-full bg-[#2B4FFF]/10 border-2 border-[#2B4FFF]/50 flex items-center justify-center font-hud text-3xl text-[#2B4FFF] glow-text">
        ...
        <div className="mt-10 grid md:grid-cols-2 gap-6">
          <section className="glass rounded-lg p-5">
            <div className="text-[10px] uppercase tracking-[0.3em] text-[#2B4FFF] mb-3">By board size</div>
              ...
              {["3", "4"].map((b) => {
                ...
                return (
                  <div key={b} className="flex items-center gap-3" data-testid={`byboard-${b}`}>
                    ...
                    <div className="flex-1 h-2 rounded-full bg-[#0A0A12] overflow-hidden border border-[#2B4FFF]/10">
                      <div className="h-full bg-[#2B4FFF]" style={{ width: `${wr}%`, boxShadow: "0 0 12px #2B4FFF" }} />
                    </div>
                    <div className="font-hud text-sm text-[#2B4FFF] w-24 text-right">{s.games} · {wr}%</div>
                  </div>
                );
              })}
            </div>
          ...
          <section className="glass rounded-lg p-5">
            <div className="text-[10px] uppercase tracking-[0.3em] text-[#2B4FFF] mb-3">By mode</div>
            <div className="space-y-2">
        ...
        <section className="mt-10">
          <div className="text-[10px] uppercase tracking-[0.3em] text-[#2B4FFF] mb-3">Recent games</div>
          <div className="glass rounded-lg overflow-hidden" data-testid="recent-games">
            ...
            {history.map((g, i) => (
              <div key={g.game_id} className="grid grid-cols-12 px-4 py-3 border-b border-[#2B4FFF]/10 items-center" data-testid={`recent-${i}`}>
                ...
                <div className="col-span-3 font-hud text-sm">
                  {g.result === "win" && <span className="text-[#00E676]">WIN</span>}
                  {g.result === "loss" && <span className="text-[#FF1744]">LOSS</span>}
